...
for loop=1:1
    ...
    for it=1:nt-2,
        ...
        d1pxz=d1px;
        p=p-dt*v.^2.*(coeff2(1)*d1pxz+coeff2(2)*d1pxz_2+coeff2(3)*d1pxz_3)/h;
        ...
        Vx=Vx-dt*(coeff2(1)*d1px+coeff2(2)*d1px_2+coeff2(3)*d1px_3)/h;
...
%% 计算解析解
delta_t = x / c;        % 时间延迟
analytical = (1 - 2*(pi*f0*(t - t0 - delta_t)).^2) .* ...
...
figure;plot(analytical,'r')
hold on;plot(seis_record,'k')
legend('Analytical2','ABS')

save('figure2_ABS_Balanced_1D_r19.mat')